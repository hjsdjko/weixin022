		...
		<view class="container" :style='{"width":"100%","padding":"0px","position":"relative","background":"#ffffff","height":"auto"}'>
            <view :style='{"padding":"24rpx 24rpx 24rpx 24rpx","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0","margin":"20rpx 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 20rpx 12rpx 0","color":"#333","borderRadius":"0","background":"#fff000","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('yuyuejiaoshi','咨询')" @tap="onAcrossTap('zixunxinxi','是','','')" class="cu-btn bg-brown round shadow-blur" >咨询</button>
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 20rpx 12rpx 0","color":"#333","borderRadius":"0","background":"#fff000","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('yuyuejiaoshi','咨询')" @tap="onAcrossTap('zixunxinxi','是','','')" class="cu-btn bg-brown round shadow-blur" >咨询</button>
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 0 0 0","color":"#fff","background":"#ffaa33","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('yuyuejiaoshi','审核')" @tap="onSHTap">审核</button>
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 0 0 0","color":"#fff","background":"#ffaa33","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('yuyuejiaoshi','审核')" @tap="onSHTap">审核</button>
				</view>
			</view>
	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('yuyuejiaoshi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yuyuejiaoshi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','yuyuejiaoshi');
			...
			async init(){
				...
				let res = await this.$api.info('yuyuejiaoshi', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussyuyuejiaoshi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
			...
			async onFinishTap() {
				...
				await this.$api.update('yuyuejiaoshi', this.detail);
				this.$utils.msg('审核成功');
